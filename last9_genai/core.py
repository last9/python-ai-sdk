...
Usage:
    from last9_genai import Last9GenAI, model_pricing
...
# No default pricing - developers must provide their own
# See PRICING.md for examples of how to add custom pricing
...
def calculate_llm_cost(
    model: str, usage: Dict[str, int], custom_pricing: Optional[Dict[str, ModelPricing]] = None
    ...
    Args:
        ...
        custom_pricing: Optional dict of model pricing. If not provided or model not found,
                       returns None (only tokens will be tracked, no cost)

    Returns:
        CostBreakdown with input, output, and total costs in USD, or None if no pricing available
    ...
    Example:
        ...
        cost = calculate_llm_cost("gpt-4o", usage, custom_pricing)
    """
    # No pricing provided - return None
    if custom_pricing is None:
        return None

    # Get model pricing
    pricing = custom_pricing.get(model)
    if pricing is None:
        # Model not in custom pricing - return None (no cost calculated)
        return None
    ...
    return CostBreakdown(
        input=round(input_cost, 6), output=round(output_cost, 6), total=round(total_cost, 6)
    )
...
class WorkflowCostTracker:
    ...
    def initialize_workflow(
        self, workflow_id: str, metadata: Optional[Dict[str, Any]] = None
    ) -> None:
        """Initialize a new workflow for cost tracking"""
        if workflow_id not in self._workflows:
            self._workflows[workflow_id] = WorkflowCost(
                workflow_id=workflow_id, metadata=metadata or {}
            )
    ...
    def add_cost(self, workflow_id: str, cost: CostBreakdown, operation_type: str = "llm") -> None:
        ...
        workflow.costs.append(
            {"cost": cost, "operation_type": operation_type, "timestamp": datetime.now()}
        )
...
class Last9GenAI:
    ...
    def __init__(
        ...
        Args:
            custom_pricing: Model pricing configuration (required for cost tracking)
                          Example: {"gpt-4o": ModelPricing(input=2.50, output=10.0)}
            workflow_tracker: Custom workflow cost tracker instance
            enable_cost_tracking: Enable automatic cost calculation (default: True)
                                 If False or no custom_pricing, only token counts tracked
        """
        self.model_pricing = custom_pricing
    ...
    def add_llm_cost_attributes(
        self, span: Span, model: str, usage: Dict[str, int], workflow_id: Optional[str] = None
    ) -> Optional[CostBreakdown]:
        """
        ...
        Returns:
            CostBreakdown with calculated costs, or None if cost tracking is disabled
        """
        # Skip cost calculation if disabled
        if not self.enable_cost_tracking:
            return None

        # Calculate cost (returns None if no pricing available)
        cost = calculate_llm_cost(model, usage, self.model_pricing)

        # Add cost attributes if we have pricing
    ...
    def add_performance_attributes(
        self,
        span: Span,
    ...
    def add_standard_llm_attributes(
        self,
        span: Span,
        model: str,
        ...
        if request_params:
            ...
            if "temperature" in request_params:
                span.set_attribute(
                    GenAIAttributes.REQUEST_TEMPERATURE, request_params["temperature"]
                )
            ...
            if "frequency_penalty" in request_params:
                span.set_attribute(
                    GenAIAttributes.REQUEST_FREQUENCY_PENALTY, request_params["frequency_penalty"]
                )
            if "presence_penalty" in request_params:
                span.set_attribute(
                    GenAIAttributes.REQUEST_PRESENCE_PENALTY, request_params["presence_penalty"]
                )
        ...
        if response_data:
            ...
            if "finish_reason" in response_data:
                span.set_attribute(
                    GenAIAttributes.RESPONSE_FINISH_REASONS, [response_data["finish_reason"]]
                )
    ...
    def add_content_events(
        ...
        if prompt:
            ...
            span.add_event(
                EventNames.GEN_AI_CONTENT_PROMPT,
                {
        ...
        if completion:
            ...
            span.add_event(
                EventNames.GEN_AI_CONTENT_COMPLETION,
                {
    ...
    def create_conversation_span(
        ...
        self.add_standard_llm_attributes(span, model, conversation_id=conversation_id)
...
def create_llm_span(
    ...
    workflow_id: Optional[str] = None,
    conversation_id: Optional[str] = None,
    l9_genai: Optional[Last9GenAI] = None,
) -> Span:
    """
    ...
    l9_genai.add_standard_llm_attributes(span, model, operation, conversation_id)
...
def create_tool_span(
    ...
    l9_genai.add_tool_attributes(span, tool_name, tool_type=tool_type, workflow_id=workflow_id)
...
def example_usage():
    ...
    with tracer.start_span("gen_ai.prompt.template") as span:
        ...
        prompt_hash = l9_genai.add_prompt_versioning(
            span, prompt_template, template_id="assistant_v1", version="1.2.3"
        )
